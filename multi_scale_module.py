...
class PAFEM(nn.Module):
    ...
    def forward(self, x, depth_weight):
        x = self.down_conv(x)
        conv1 = self.conv1(x)
        conv2 = self.conv2(x)
        ...
        # energy2_new=energy2.view(m_batchsize, 1, width * height, width * height)
        # depth_weight_new = depth_weight.view(m_batchsize, 1, width * height, width * height)
        energy2=energy2+depth_weight
        # energy2_u = self.fuseout(torch.cat((energy2_new, depth_weight_new), 1))
        # energy2 = energy2_u + energy2_new
        # energy2=energy2.view(m_batchsize, width * height, width * height)
        ...
        #energy3=torch.multiply(energy3,depth_weight)
        # energy3_new = energy3.view(m_batchsize, 1, width * height, width * height)
        # depth_weight_new = depth_weight.view(m_batchsize, 1, width * height, width * height)
        energy3=energy3+depth_weight
        # energy3_u = self.fuseout(torch.cat((energy3_new, depth_weight_new), 1))
        # energy3 = energy3_u + energy3_new
        # energy3 = energy3.view(m_batchsize, width * height, width * height)
        ...
        # #energy4=torch.multiply(energy4,depth_weight)
        # energy4_new = energy4.view(m_batchsize, 1, width * height, width * height)
        # depth_weight_new = depth_weight.view(m_batchsize, 1, width * height, width * height)
        energy4=energy4+depth_weight
        # energy4_u = self.fuseout(torch.cat((energy4_new, depth_weight_new), 1))
        # energy4 = energy4_u + energy4_new
        # energy4 = energy4.view(m_batchsize, width * height, width * height)
